...
export default function QuestionTab({ formDetails }) {
  ...
  // handleEditAddQuestion(question[i], 'edit_question')
  ...
  const handleEditAddQuestion = async (quiz, type) => {
    ...
    if (type === "edit_question") {
      ...
      for (let f = 0; f < Object.values(all_data).length; f++) {
        if (
          Object.values(all_data)[f] !== null &&
          Object.values(all_data)[f] !== undefined &&
          Object.values(all_data)[f] !== ""
        ) {
          await formEditData.append(
      ...
      await axiosInstance
        .post(`/question/${all_data.id}`, formEditData, formTokenConfig())
        .then(async (res) => {
      ...
      for (let f = 0; f < Object.values(all_data).length; f++) {
        console.log(Object.values(all_data)[f]);
        console.log("---------", Object.values(all_data)[f] !== null);
        console.log("---------", Object.values(all_data)[f] !== undefined);
        console.log("---------", Object.values(all_data)[f] !== "");
  ...
  async function copyQuestion(i) {
    ...
    var newQuestion = {
      ...
      mcq_many: [...qs[i].mcq_many],
      mcq_one: [...qs[i].mcq_one],
  ...
  //     optionsOfQuestion[i].background_image = link;
  //   } else {
  //     var j = context.option;
  //     optionsOfQuestion[i].options[j].background_image = link;
  //   }
  ...
  function handleImageValue(i, bg) {
    var optionsOfQuestion = [...questions];
    optionsOfQuestion[i].background_image = bg;
    setQuestions(optionsOfQuestion);
  }
  function handleTextImage(i, bg) {
    var optionsOfQuestion = [...questions];
    if (optionsOfQuestion[i].question_type === "txt") {
      optionsOfQuestion[i].txt.background_image = bg;
    } else if (optionsOfQuestion[i].question_type === "lng_txt") {
      optionsOfQuestion[i].lng_txt.background_image = bg;
    }
    setQuestions(optionsOfQuestion);
  }
  function handleImageValueOption(i, j, bg) {
    var optionsOfQuestion = [...questions];
    if (optionsOfQuestion[i].question_type === "mcq_many") {
      optionsOfQuestion[i].mcq_many[j].background_image = bg;
    } else if (optionsOfQuestion[i].question_type === "mcq_one") {
      optionsOfQuestion[i].mcq_one[j].background_image = bg;
    }
    setQuestions(optionsOfQuestion);
  }
